    ...
    protected JComponent createDisclosedComponent(int state) {
        ...
        switch (state) {
            case 1: 
                ...
                gbc.insets = new Insets(3, 0, 0, 0);
                p.add(btn, gbc);
                btn = ButtonFactory.createFontStyleSuperscriptButton(editor, labels, disposables);
                btn.setUI((PaletteButtonUI) PaletteButtonUI.createUI(btn));
                btn.putClientProperty("Palette.Component.segmentPosition", "middle");
                gbc = new GridBagConstraints();
                gbc.gridy = 2;
                gbc.insets = new Insets(3, 0, 0, 0);
                p.add(btn, gbc);
                btn = ButtonFactory.createFontStyleSubscriptButton(editor, labels, disposables);
                btn.setUI((PaletteButtonUI) PaletteButtonUI.createUI(btn));
